...
class AlertRecipientServer(BaseRouter):
    ...
    async def get_list(
        ...
        query = self.pg_connector.session.query(AlertRecipient)
    ...
    def add_new_alert_recipient(self, alert: dict = Body()) -> JSONResponse:
        """
        ...
        alert_recipient = self.get_all(
            filter={"display_name": alert.display_name}
        )
        ...
        self.pg_connector.session.add(alert_recipient)
    ...
    async def delete_alert_recipient(self, alert_recipient_id: int):
        """
        Удаляет alert_recipient, по его id
        Args:
            alert_recipient_id: id получателя уведомлений
        """
        dataset = self.get_by_id(alert_recipient_id)
    ...
    async def update_project(
        self, alert_recipient_id: int, alert: dict = Body()
    ) -> JSONResponse:
        """
